/* ============================================================
   ÁLGEBRA PARA TODOS · Unidad 1 · Actividad 5
   ...
   En la landing de Kajabi va únicamente esto, en este orden:
     ...
     <script src="https://algebraparatodos.github.io/problemas-tomo-2/actividades/unidad-1-actividad-5-escalonada-reducida.js"><\/script>
   ============================================================ */
(function () {
  /* El div de montaje se inserta ACÁ, en el lugar del <script>,
     aprovechando que document.currentScript todavía es válido. */
  (function placeMount() {
    if (document.getElementById('apt-u1a5')) return;
  ...
  (function(){
    ...
    // ---------- Verificador: única fuente de verdad ----------
    ...
    function breakEchelon(rows){
      ...
      if(kind === 'zero-not-last'){
        ...
        if(r[zeroIdx+1].every(function(v){ return v===0; })){
          r[zeroIdx+1][randInt(0,COLS-1)] = randNonZero(1,9);
        }
        return r;
      } else {
    ...
    function generateMatrix(){
      ...
      var rows;
      if(recipe === 'clean-rref'){
        rows = cleanPivotColumns(buildSkeleton(function(){ return 1; }));
      } else if(recipe === 'break-structure'){
        rows = breakEchelon(buildSkeleton(function(){ return 1; }));
      } else if(recipe === 'pivot-value'){
        rows = cleanPivotColumns(buildSkeleton(function(){ return randPivotValNotOne(); }));
      } else {
        rows = buildSkeleton(function(){ return 1; });
      }
    ...
    function highlightLatex(rows, v){
      var body = rows.map(function(row, ri){
        if(!v.ok && ri === v.badRow){
          return row.map(function(x){ return '\\textcolor{#D65252}{'+x+'}'; }).join(' & ');
        }
        if(v.ok){
          var p = pivotOf(row);
    ...
    window.AptActivity.init({
      ...
      subtitle: 'Mirá la matriz y decidí si está en su forma escalonada reducida por filas.',
      nextLabel: 'Probar con otra matriz →',
      needsKatex: true,
      mode: 'choices',
      choices: [
        { value:'si', label:'Sí, es reducida' },
        { value:'no', label:'No, no es reducida' }
      ],
      generate: function(){
